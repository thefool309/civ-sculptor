...
#include <string>
/// <summary>
/// ResourceType is an enumeration class that defines various types of
/// resources, such as Food, Water, and Metal, using an underlying type of
/// uint8_t to represent them efficiently.
/// </summary>
...
static_assert(std::size(Const::ResourceTypeNames) ==
                  static_cast<size_t>(ResourceType::COUNT_RT),
...
struct Resources {
  ...
  Resources(ResourceType _type, uint64_t _amount)
...
struct Debt {
  ...
  Debt(std::string _civID, Resources _resource)
      : resource(_resource), civID(_civID) {}

  Debt(std::string _civID, ResourceType type, unsigned int amount)
      : civID(_civID), resource(Resources(type, amount)) {}
};