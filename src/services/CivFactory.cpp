/// @file CivFactory.cpp
/// @brief CivFactory implementations - seed generation, civ creation, batch
/// loading
...
CivFactory::ReadCivNamesFromTxt(std::string &_filepath) {
  std::ifstream fin(_filepath);
  std::vector<std::string> civNames;
  if (!fin.is_open()) {
    throw std::runtime_error(
        "file failed to open! check filepath!"); // throw runtime_error if
                                                 // failed to open
  }
...
CivSeed CivFactory::GenerateCivSeed(std::string name, uint64_t _worldSeed) {
  ...
  uint64_t hash =
      Const::FNV_offset_basis; // the hash starts as the offset basis
  ...
  hash ^= _worldSeed; // xor the world seed into it one time to make it
                      // deterministic
  hash *= Const::FNV_prime;
...
Civilization CivFactory::CreateCivilization(std::string name,
                                            uint64_t _rngSeed) {
  CivSeed seed = GenerateCivSeed(name, _rngSeed);
  return Civilization(++currentCivId, seed.name, seed);
}

std::unordered_map<std::string, Civilization>
CivFactory::GenCivsFromList(std::string _filepath, uint64_t _rngSeed,
                            RandomEngine rng) {
  std::vector<std::string> civNames;
  try {
  ...
  for (auto &name : civNames) {
    result.insert(std::pair<std::string, Civilization>(
        name, CivFactory::CreateCivilization(name, _rngSeed)));
  }
